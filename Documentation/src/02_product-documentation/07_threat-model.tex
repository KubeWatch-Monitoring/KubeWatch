...
\subsection{Database}
\begin{itemize}
    \item We use MongoDB as a database, which implements NoSQL.
\end{itemize}
...
\begin{itemize}
    ...
    \item They implement their own query language, PromQL, which is subject to injection attacks if input is not properly escaped. Currently, only the backend submits hard-coded queries to the database.
\end{itemize}