...
At each push from our project to the gitlab environment we use the CI/CD pipeline to make some checks and generating the pdf.

Our pipeline consists of three stages: \textit{build, test, deploy}. Each of these stage is described in the following section in more detail.

\subsection{Pipeline Stages}
\textbf{Build:} This is the first pipeline stage and here the documentation pdf will be build. In addition, this stage is also used to build the docker image which is used to publish our application and is also used in the deployment stage to deploy our application to the cluster.


\textbf{Test:} This is our second pipeline stage and is used to test our application. We automated all tests we could to minimize the effort each teammember need to make for the tests. The advantage of this method is definitively that the tests never get forgotten. We can perform the \textit{Unit Tests} and the \textit{Integration Tests} within our test stage of the ci/cd pipeline.


\textbf{Deploy:} In our last pipeline stage we deploy our docker image to the cluster which is located in the INS network.

\subsection{Accessing Deployed Application}
To access the Kubernetes cluster in the INS, you need first to access via VPN to the INS network. Then you will find the web application in the \textit{Deployments -> Environments} and there you can \textit{Open} the web application. \newline
